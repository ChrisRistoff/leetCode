...
that each substring contains exactly one unique digit. Then for each substring,
...
function countAndSay(n: number): string {
  ...
  function say(s: string): string {
    ...
    while (i < s.length) {
      ...
      result.push(count.toString() + s[i]);
      i++;
    }
    return result.join('');
  }
...
// time: O(n * m) where n is the input number and m is the length of the string
